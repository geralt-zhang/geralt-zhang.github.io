...
几乎所有您能够接触到的平台都支持某种形式的 shell，有些甚至还提供了多种 shell 供您选择。虽然它们之间有些细节上的差异，但是其核心功能都是一样的：它允许你执行程序，输入并获取某种半结构化的输出。因此，事实上，Shell可以看作一个执行程序的程序，具体而言，通过 *fork() & exec()* 调用执行程序。
...
```console
missing:~$ date
Fri 10 Jan 2025 11:49:31 AM EST
missing:~$ 
```
